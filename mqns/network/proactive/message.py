...
class PathInstructions(TypedDict):
    ...
    The semantics of "age" depend on the CutoffMode passed to ProactiveForwarder.

    When it's a node's turn to perform swapping, EPRs involved in the swap must have ages
    no greater than the specified duration. If an in-memory EPR has exceeded the age limit of all
    applicable paths, it should be released to make room for a new EPR.
