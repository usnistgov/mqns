...
def test_write_and_read_with_path_and_key():
    ...
    addrs = mem.allocate(0, PathDirection.LEFT, ch=ch)
    assert len(addrs) == 1
    addr = addrs[0]
...
def test_memory_clear_and_deallocate():
    ...
    # Test deallocate
    addrs = mem.allocate(7, PathDirection.LEFT, ch=ch)
    assert len(addrs) == 1
    addr = addrs[0]
    mem.deallocate(addr)
...
def test_qubit_reservation_behavior():
    ...
    addrs = mem.allocate(42, PathDirection.LEFT, ch=ch)
    assert len(addrs) == 1
    addr1 = addrs[0]
